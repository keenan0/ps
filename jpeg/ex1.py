...
encoded_bistream, table, shape = jpeg_encode(X)
decoded_img = jpeg_decode(shape, encoded_bistream, table)

pixel_error = (X - decoded_img) ** 2
pixel_error = np.log10(pixel_error + 1e-2)

print(f"Compression rate: { (X.shape[0] * X.shape[1] * 8) / len(encoded_bistream) :.2f}:1")
print(f"MSE: {np.mean((X - decoded_img)**2):.2f}")

plt.figure(figsize=(12, 6))
plt.subplot(131).imshow(X, cmap='gray'); plt.title("Original")
plt.subplot(132).imshow(decoded_img, cmap='gray'); plt.title("JPEG + Huffman")
plt.subplot(133).imshow(10 * pixel_error, cmap='hot'); plt.title('Log Error Heatmap')
plt.show()